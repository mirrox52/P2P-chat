...
void ListenBroadcastUDPThread(ULONG IP, BOOL &Work, PList &ClientList, std::mutex &mtx, char* &Nickname)
{
	...
	timeval Delay = { 5, 0 };
	fd_set UDPSocket = { 1, sock }; //структура с кол-вом сокетов и массивом сокетов

	while (Work)// слушать юдп запросы
	{

		Res = select(0, &UDPSocket, NULL, NULL, &Delay);// тестирует сокет на читаемость
		// слушаем на наличие соединений
		if (Res == 0)
		{
		...
		if (Res > 0)
		{
			...
			recv(sock, (char*)&NewClient->IP, sizeof(ULONG), 0); // сокет буффер длина
			...
			else
			{
				...
				ThreadPooll.push_back(std::thread(ReciveData, std::ref(Work), std::ref(NewClient), std::ref(ClientList), std::ref(mtx), std::ref(Nickname))); //добавляем в конец вектора поток с параметрами
